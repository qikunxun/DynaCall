...
_MOVIE_POLICY = """Return exactly one JSON array and nothing else.
Top-level format: [{"kind":"tool",...}, ...].
Allowed node kinds: tool, join.

Node formats:
- tool: {"kind":"tool","tool":"search","args":[...]}
- tool with reusable id: {"kind":"tool","id":"name","tool":"search","args":[...]}
- join: {"kind":"join"}

Rules:
1. Output valid JSON only.
2. Allowed tool: search.
3. Use named ids like $q1, $opt1. Do not use positional ids like $1.
4. Keep exactly one final join at the end of the whole plan.
5. Find all movies in the Question and Options before join.
"""

_MOVIE_FEW_SHOTS = """Question: Find a movie similar to Mission Impossible, The Silence of the Lambs, American Beauty, Star Wars Episode IV - A New Hope
Options:
...
Rosetta
[
  {"kind":"tool","id":"q1","tool":"search","args":["Mission Impossible"]},
  {"kind":"tool","id":"q2","tool":"search","args":["The Silence of the Lambs"]},
  {"kind":"tool","id":"q3","tool":"search","args":["American Beauty"]},
  {"kind":"tool","id":"q4","tool":"search","args":["Star Wars Episode IV - A New Hope"]},
  {"kind":"tool","id":"opt1","tool":"search","args":["Austin Powers International Man of Mystery"]},
  {"kind":"tool","id":"opt2","tool":"search","args":["Alesha Popvich and Tugarin the Dragon"]},
  {"kind":"tool","id":"opt3","tool":"search","args":["In Cold Blood"]},
  {"kind":"tool","id":"opt4","tool":"search","args":["Rosetta"]},
  {"kind":"join"}
]
###"""

PLANNER_PROMPT = _MOVIE_POLICY + "\nExamples:\n\n" + _MOVIE_FEW_SHOTS

PLANNER_PROMPT_REPLAN = _MOVIE_POLICY + """

Replanning rules:
1. Produce a fully self-contained new JSON array.
2. Do not reference ids from the failed plan.
3. Rewrite weak movie queries with canonical title, release year, or explicit '(film)'.
4. Keep exactly one final join.
"""
...
OUTPUT_PROMPT = (
    ...
    f"  - You MAY include a brief `Thought:` line before the final action.\n"
    f"  - You MUST end with exactly one final line: `Action: {JOINNER_FINISH}(answer)`.\n"
    "  - Do NOT output any extra Action lines besides the final Finish action.\n"
